...
export function instagramResponse () {
  return response;
}
...
export function filterList() {
  var allFilters = response.data.map(function(photo){
    return photo.filter;
  });

  return allFilters.filter(onlyUnique);
}
...
export function tagList() {
  ...
  for (var i = 0; i < photos.length; i++){
    allTags = allTags.concat(photos[i].tags)
  }
